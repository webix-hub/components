...
tinymce.util.Quirks = function(d) {
	...
	function F() {
		function G(J) {
			...
			I.appendChild(K);
			return E.serializer.serialize(I, {
				format: "html"
			})
		}
		function H(I) {
	...
	function C() {
		...
		function G() {
			return !E.isCollapsed() && E.getStart() != E.getEnd()
		}
		function I(J, K) {
			K.preventDefault();
			return false
	...
	function g() {
		...
		d.contentStyles.push(".mceResizeImages img {cursor: se-resize !important}");
		...
		function L(R) {
			...
			if (K) {
				O = R.screenX - H;
				N = R.screenY - G;
				Q = Math.max((I + O) / I, (J + N) / J);
				if (Math.abs(O) > 1 || Math.abs(N) > 1) {
					P = Math.round(I * Q);
					M = Math.round(J * Q);
					if (K.style.width) {
						p.setStyle(K, "width", P)
					} else {
						p.setAttrib(K, "width", P)
					}
					if (K.style.height) {
						p.setStyle(K, "height", M)
					} else {
						p.setAttrib(K, "height", M)
					}
					if (!p.hasClass(d.getBody(), "mceResizeImages")) {
						p.addClass(d.getBody(), "mceResizeImages")
					}
				}
			}
		}
		d.onMouseDown.add(function(M, O) {
			var N = O.target;
			if (N.nodeName == "IMG") {
				K = N;
				H = O.screenX;
				...
				I = K.clientWidth;
				J = K.clientHeight;
				p.bind(M.getDoc(), "mousemove", L);
				O.preventDefault()
			}
		});
		d.onNodeChange.add(function() {
			if (K) {
				K = null;
				p.unbind(d.getDoc(), "mousemove", L)
			}
			if (E.getNode().nodeName == "IMG") {
				p.addClass(d.getBody(), "mceResizeImages")
			} else {
				p.removeClass(d.getBody(), "mceResizeImages")
			}
		})
	}
	A();
		...
		function g(A) {
			...
			function F(N) {
				...
				if (b.isIE && i.isBlock(N)) {
					M = e.getRng();
					N.appendChild(i.create("span", null, "\u00a0"));
					e.select(N);
